...
const search = async (driver, searchTerm) => {
  ...
  console.log('resultText: ', resultsText);
...
const searchTwo = async (driver, searchTerm) => {
  ...
  //   sendKeys() inputs the searchTerm into the search bar
  await driver.findElement(By.name('q')).sendKeys(`${searchTerm}\n`);

  // choose the correct tag
  await driver.findElement(By.xpath('//a[text()="Images"][1]')).click();

  // change search term specific to my google results to pass
  let resultsText = await driver
    .findElement(By.xpath('//h3[contains(text(),"Newborn Puppies")]'))
    .getText();

  expect(resultsText.toLowerCase()).toContain('newborn puppies');
};
