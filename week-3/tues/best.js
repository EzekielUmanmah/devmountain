// This challenge, you’ll write a function, called best, that returns the maximum profit possible for buying-and-selling
// a stock.
...
function best(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (max - data[i] > maxProfit) {
      maxProfit = max - data[i];
      buy = data[i];
      sell = max;
    }
  }

  const result = `${maxProfit} - buy at $${buy}, sell at $${sell}`;
